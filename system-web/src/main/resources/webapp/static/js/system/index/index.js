...
layui.config({
    ...
    "httpAjax" : "httpAjax"
}).use(['bodyTab','form','element','layer','jquery','httpAjax'],function(){
    let form = layui.form,
        layer = layui.layer,
    ...
    tab = layui.bodyTab({
        ...
        url : urlPath+"/menu/queryMenu?menuType=menu" //获取菜单json地址
    });
    //更换皮肤
    function skins(){
    ...
    $('body').on('click', '[data-check-screen]', function () {
        ...
        if (check == 'full') {
            ...
            $(this).html('<i class="layui-icon layui-icon-screen-restore"></i><cite>退出全屏</cite>');
        } else {
            exitFullScreen();
            $(this).attr('data-check-screen', 'full');
            $(this).html('<i class="layui-icon layui-icon-screen-full"></i><cite>全屏</cite>');
        }
    });
    //全屏
    function fullScreen(){
    ...
    function lockPage(){
        layer.open({
            ...
            content : '	<div class="admin-header-lock" id="lock-box">'+
                '<div class="admin-header-lock-img"><img src="images/userface.jpg"/></div>'+
                '<div class="admin-header-lock-name" id="lockUserName">页面锁屏</div>'+
                '<div class="input_btn">'+
    ...
    //公告层
    function showNotice(){
        let pageNo = 1,pageSize = 1,content,url= urlPath + "/notice/findNotice?v=" + Math.random(),
            param={
                page:pageNo,
                limit:pageSize,
                noticeType:'system'
        ...
        if(res!=null){
            ...
            if(code ==0){
                ...
                if(data!=null&&data.length==1){
                    content = data[0].noticeContent;
                }else{
                    content = "welcome to online system ！！！";
                }
                layer.open({
                    type: 1,
                    title: "系统公告",
                    ...
                    content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; ' +
                        ' text-indent:2em;border-bottom:1px solid #e2e2e2;">' +content+
                        ' </div>',
